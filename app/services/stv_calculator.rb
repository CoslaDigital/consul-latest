class StvCalculator
  ...
  def calculate(ballot_data, seats, quota, investment_titles)
    ...
    @elimination_log = []
    rounds_log = [] # This will store the data for each round
    empty_seats = seats
    iteration = 1
    ...
    loop do
      ...
      # Log the state of the round before taking action
      current_round_data = {
        iteration: iteration,
        quota: quota,
        standings: sorted_investments,
        action: nil # We'll fill this in next
      }
      ...
      if elected_in_round.present?
        ...
        details: { 
          ...
          exhausted_count: exhausted_count # <-- Store the exhausted count
        } 
      }        
      end
